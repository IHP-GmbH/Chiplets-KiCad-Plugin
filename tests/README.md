...
| `test_orchestrator.py` (63) | The pure helpers in `pipeline/orchestrator.py` that the dialog cannot exercise otherwise: `build_cli_args`, `build_adk_drc_argv`, `build_worker_env`, `load_interposer_adapter` / `load_interconnect_adapter`, `available_connection_types`, `discover_dependency_root`, `discover_interposer_lyp`, `derive_interconnect_methods` + `write_ixn_methods_sidecar`, `_read_component_connections`, `describe_assembly_drc`, and `ExportOptions` / `ExportResult` DRC defaults. |
